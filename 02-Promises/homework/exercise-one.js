...
function problemC() {
  ...
  // ???
  let files = [];
  files.push(promisifiedReadFile("poem-one/stanza-02.txt"));
  files.push(promisifiedReadFile("poem-one/stanza-03.txt"));
  Promise.all(files).then((stanzas) =>
...
function problemE() {
  ...
  let files = [];
  files.push(promisifiedReadFile("poem-one/stanza-03.txt"));
  files.push(promisifiedReadFile("poem-one/stanza-04.txt"));
  Promise.all(files).then(
    (stanzas) => stanzas.forEach((stanza) => blue(stanza)),
    (err) => magenta(err)
  );
}
