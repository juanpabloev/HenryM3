...
function problemA() {
  ...
  // promise version
  // ???
  let stanza1 = promisifiedReadFile("poem-two/stanza-01.txt").then((stanza) =>
    blue(stanza)
  );
  let stanza2 = promisifiedReadFile("poem-two/stanza-02.txt").then((stanza) =>
    blue(stanza)
  );

  Promise.all([stanza1, stanza2]).then(() => console.log("done"));
}
...
function problemB() {
  ...
  // promise version
  // ???
  let promesArray = filenames.map((file) => {
    return promisifiedReadFile(file).then((stanza) => blue(stanza));
  });
  Promise.all(promesArray).then(() => console.log("done"));
}
